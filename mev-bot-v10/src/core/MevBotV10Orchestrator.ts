...
export class MevBotV10Orchestrator {
    ...
    private async processSimulationResult(simResult: SimulationResult): Promise<void> {
        ...
        const executionEnabled = this.configService.get('EXECUTION_ENABLED') === 'false';
        ...
        else if (simResult.blockAgeCheckFailed && this.currentBlockNumber !== 0) discardReason = "Block age check failed (source tx too deep)";
